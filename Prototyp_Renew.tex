...
\subsection{Gradle}
	...
	Ant spielt eine unerwartet wichtige Rolle für das erstlen der \textsc{Renew} Plugins. Denn, Ant verwaltet nicht nur das Kompilieren und Erstellen von ausführbaren Archiven, sondern ist zuständig für die Verwaltung der zusätzlichen Ausführungschritte, die \textsc{Renew} Code sowie Resourcen generieren. Zum Teil sind es Drittanbieter Werkzeuge, wie der \textit{JavaCC} Compile und zum Teil speziel für \textsc{Renew} entwickelte Verfahren. \bigbreak
	...
	Andere Plugins benötigen keine Drittanbieter Software, sondern beruhen auf eigenen Verfahren, die für die Plugins ausführbaren Code und Artefakte heerstellen. Für die Umsetzung der Ausführungsschritte, wurden an Ant angepassten Java Klassen geschrieben, die auf der \textsc{Renew}-Logik beruhen und von Ant aufgerufen und durchgeführt werden können. Inflogedesse stellt sich die Frage wie die Ausführungsschritte aus der Ant in die Gradle Umgebung migriert werden könne. \newline
	Um die Anwender von Ant und Maven von Gradle zu überzeugen, integriert Gradle beide Systeme und lässt ihre Funktionalität aus Gradle benutzen und steuern. Somit ist ein  nahtlosen übergangn auf Gradle garantiert und überzeugt die Anwender durch die mitgelieferte flexibilität.\bigbreak
	
	Um die erstellten \textit{Renew} Verfahren in Gradle zu übernehmen, kann auf eine mitgelieferte \textit{ant} Instanz aus Gradle zugegriffen und für die Konfiguration der Verfahren aufbereitet werden. Anschließend kann über die \textit{ant} Instanz die konfigurierte Logik aufgerungen und ausgeführt werden. Obwohl das Konstrukt beherrschbar erscheint, erfordert dies trotz Allem eine gute Kenntnis der Ant Funktionalität und führt zu irreführenden Annahmen bei unerfahrenen Entwicklern. 
...
\subsection{Modularisierung}	
	...
	\begin{figure}[h!]
	  \centering
	  \includegraphics[scale=0.42, angle=90]{material/images/renew_plugin_dependencies-module-info.pdf}
...
\subsection{Endergebnis} \label{sub:endergebnis}
	...
	\begin{figure}[h!]
	  \centering
	  \includegraphics[width=0.63\textwidth]{material/images/renew_plugin_dependencies-migrate_opt.pdf}
