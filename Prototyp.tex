...
% Ablesen der Plugin cfg's 
	...
	% ÜBERBLICK: Plugin.cfgs ablesen groben überblick verschaffen
	% AUSPROBIEREN: mit dem target Settler bauen und schauen ob dieses funktioniert. 
	% ERWARTUNG: dieses muss alle benötigten Mulan und Renew Plugns in den build miteinbezihen. 
	% FEHLER: nicht alle nötigen plugins sind in den plugin cfgs enthalten
	% ANALYSE: Es besteht eine Abhängigkeit zwischen den de.renew.agent.transport und den de.renew.agent.subscriptionmonitor Plugn 
	% BEHANDLUNG: Diese werden in die Plugncfgs nachgezogen und in den Ant skripten verankert, nähmlich im Kern Mulan Plugn CAPA,
	% ERGEBNISS: Mulan Plugn Menge hat zugenommen 
	\begin{figure}[h!]
	  \centering
	  ...
	  \label{fig:trans_config}
	\end{figure}
	% FOLGERUNG: Die Renew Plugnmenge nimmt auch zu.
	% BEHANDLUNG: Modularisierung von zusätzlichen  Renew Plugns  
	\begin{figure}[h!]
	  \centering
	  ...
	  \label{fig:trans_config}
	\end{figure}	
	% AUSPROBIEREN: Ausführen des Settler targets 
	% FEHLER: Die Pfad Abhängigkeiten von Settler und Ant entspricht nicht dem modularen Renew 
	% ANALYSE: modularRenew Klassen besitzen zum teile eine andere Packetstruktur
	% BEHANDLUNG: die Benötigten Klassen werden in den Settler und Ant Skripten nachgepflegt 
	% AUSFÜHRUNG: Plugns anpassung in -> UseCaseComponents,YamlToFSConceptDiagram,KnowledgeRoundtrip,MulanDoc,FSOntologyGenerator,AgentRoleModeler
	% AUSFÜHRUNG: in Ant mulantargets.xml anpassen 
	% AUSPROBIEREN: Der Setterl task wird wider verwendetz
	% FEHLER: Settler kann nicht auf UI Methoden des Gui Plugns zugreifen.
	% BEHANDLUNG: Wurde mit einer Abschluss arbeit überarbeitet und nicht  in mulan angepasst. 
	% AUSFÜHRUNG: GUI Plugn analyse und anpassung des Zugriff auf die das neue Gui Plugin 
	% AUSPROBIEREN: Start der Mulan Rewn Applikation 
	% FEHLER: Schadow netze könen nicht geladen werden.
	% BEHANDLUNG: diese tragen metainforation über ihren Compiler aus RRenew und müssen angepasst werden.
	% BEHANDLUNG: mit hilfe der regulären ausdrücke alle Shadow netz metainformation  mit dem neuen Kompiler ersetzen. 
	% AUSPROBIEREN: Start von Mualn 
	% FEHLER: das Settler Shadownetz wird nicht gefunden.
	% ANALYSE: Dieses wird normalerweise nicht mitgelifert, sondern mit eine bash startcrit referenziert. 
	% BEHANDLUNG: Für uns ist Settler das Ziel daher wird dieses netz mit Kompeliert und  im Settler plugn verpackt. 
	% VERTIG:
	\begin{figure}[h!]
	  \centering
