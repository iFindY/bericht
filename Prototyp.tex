...
\subsection{Gradle}
 % gradle build script erstellen
	...
	Zum Schluss der globalen Konfiguration wird ein \textit{jar} Task angelegt, der für ein gegebenes \textit{Source Set} ein \textit{jar}-Archiv für jedes Plugin mit den dazugehörigen Ressourcen erstellt. Zu diesem Zeitpunkt sind alle Subprojekte eingebunden, dennoch fehlt diesen die individuelle Konfiguration.
	\bigbreak
	...
	Jedes einzelne Plugin wird auf diese Weise konfiguriert und enthält einen Namen sowie zusätzliche Abhängigkeiten. Damit ist die Vorbereitung für die Modularisierung abgeschlossen. 
...
\subsection{Modularisierung}	
	...
	Der neu entstandenen Graph wurde im Vergleich zu dem Graphen aus dem Ausgangssituation Abschnitt \ref{fig:plugin_deps} durch ein Plugin erweitert und bindet alle Plugins an das \textit{Loader} Projekt.
	\bigbreak
	\begin{figure}[h!]
	  \centering
	...
	Laut der Abbildung \ref{fig:deps} sind keine Zyklen in der minimalen Version vorhanden und demnach muss keine weiteren Anpassungen durchgeführt werden. 
	\bigbreak
	...
	Jedes Module benötigt eine \textit{module-info.java}, die alle Java internen sowie Drittanbieter Bibliotheken auflistet. Für die ersten Module werden die erforderlichen Bibliotheken inklusive dem Loader Plugin in der Konfigurationsdatei mit dem Schlüssel \textit{requiers} verankert und die dazugehörigen Drittanbieter-Bibliotheken auf dem Modulpfaden als automatische  Module eingebunden.
	\begin{figure}[h!]
	  \centering
	...
	Damit sind die notwendigen Schritte für die Modularisierung bestimmt worden und können in einem Zyklus, bis alle Plugins auf dem Modulpfaden befinden, durchgeführt werden. Nähmilch das Auslesen der Plugin Kompilation- sowie Laufzeit Abhängigkeiten aus dem Gradle Build Skript und das Nachrüsten der Schnittstellen bestehender Module. 
	\bigbreak
...
% Analyse der Entstandenn Abhängigkeiten 
	% Bild
% Zwei Pluginarten compile und laufzeit. manche müssen für die ausführung mitgeliefert werden. 
...
%  der graph ist vereinfacht und kann vereinfacht in der java-info abgebildet werden durch static und transitiv 
