...
\section{Folgen} \label{sec:auswirkung}
	...
	% Das Plugin Konzpet 
		Eine Plugin-Architektur ist eine Architektur, die externen Code aufruft, ohne alle Details der Codebasis im Voraus zu kennen. Um eine Kommunikation mit dem Kern aufnehmen zu können, müssen die Plugins einer Konvention folgen. Die Konvention wird als ein \textit{interface} umgesetzt welches beiden Kommunikationspartner umsetzen. Das UML-Diagramm \ref{fig:pluginUMLD} veranschaulicht Kommunikationsvertrag zwischen den Plugins und dem Plugin-Manager. \bigbreak
		\begin{figure}[h!]
			\centering
	...
	% Der Plugin Manager % einer der aufgaben 
		...
		\subsubsection{Das Klassenlader System von \textsc{Renew}}
			...
			In der ersten Instanz wird die Basis etabliert, die den Plugin Manager sowie die Drittanbieter-Bibliotheken einliest und versorgt damit den darunter liegenden Plugin-Klassenlader mit Funktion. Der darunter liegende Plugin-Klassenlader lädt anschließend alle Plugins nach einender in der richtigen Reihenfolge ein und garantiert das Vorkommen jeder benötigten Plugin Klasse auf dem Klassenpfad, wenn diese benötigt wird. Zuletzt wird ein Dynamischer Klassenlader erstellt, der für das Simulieren der Petrinetze entsprechende Klassen und Ressourcen beim Ausführen einliest. \newline
			Die Abbildung der ausformulierten Klassenlader Systems von \textit{Duvigneau} wird in der Abbildung \ref{fig:classLoadDuv} dargestellt. \newline
			% Bild Klassloader 
			\begin{figure}[h!]
				\centering
