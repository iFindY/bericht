...
\section{Auswirkung} \label{sec:auswirkung}
	...
	\subsection{Verwaltung} \label{sub:verwaltung}
		Das Plugin System von \textsc{Renew} besteht aus eine großen Anzahl an Plugins, die Zweckorienttier kombiniert werden können. Demnach besteht bereits ein Organisationsaufwand, der mit jeder Plugin Kombination wächst. Mit der Einführung der Java Module werden Aufgabenmengen aus einer Code-Struktur in mehrere Code-Bausteine zerlegt und müssen dementsprechend disjunkt verwaltet werden. Somit müssen größere \textsc{Renew} Plugins, für die saubere Umsetzung, auf Module aufgeteilt und makellos organisiert werden. Denn, das Arbeiten mit einer variablen Anzahl an Modulen stellt das Grundprinzip des Modul Systems von Java dar.\bigbreak

		Die Herausforderung dynamische Modulkombination aus einer großen Menge zu erstellen, spielt eine zentrale Rolle in einem Modulverband. Daher ist eine externe Administration ein wichtiges Werkzeuge für das Entwickeln von Modulen. Für die Verwaltung der Codebasis existieren bereits Werkzeuge wie \textit{Ant}, \textit{Maven} und \textit{Gradle}. Da \textit{Gradle} die besten Eigenschaften von \textit{Maven} und \textit{Ant} in sich trägt und zusätzliche Vorzüge zum Verwalten, Bauen und Verbreiten von Code besitzt, ist dieses bestens für das Modulsystem gewappnet. \bigbreak
	    Die zentrale Aufgabe besteht aus dem Bauen von kompatiblen Plugins mit den geeigneten Modulen, die über Konfigurationsdateien, Benutzereingaben, und äußeren Einflüssen gesteuert und ausgeführt werden müssen.
