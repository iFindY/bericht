...
- modulsichtbarkeit und -abhängigkeit in java 9 eingeführt 
- In der praktischen Umsetzung werden die Schnittstelle durch die module-info formuliert und enthält drei Koppluingsarten 
- require, export ,open 
- diese können zusätzlich konfiguriert werden 
! <hier ist ein Bild, das die Struktur darstellt>
- Dementsprechend dienen die Kopplungsschlüssel nicht nur der Lesbarkeit, sondern erweiterern die Prozedur des Klassenladens durch explizite Zugrifsberechtingungen. 
- Java 9 besitzt immer noch die Klassloader Hierarchie mit den Boot-,dem umbenannten Platform und Applikationsloader.
- In den modernisierten JDK  übernehmen sie das laden bestimmter Module anstelle der ehemaligen jars.
!  <bild unterteilt module auf die drei  klassloader> 
- Rückblick auf classpath kapitel 
- Java 9 wurde modularisiert und hat es vorgezeigt, indem der JDk modularisiert wurde und mit Hilfe eines Abhängigkeitsgraphen  die nötigen KModule in die Applikation eingebunden werden können. Diese könne dierekte, transetive oder offenen Verbindungen beinhalten. Wichtige eingeschaft des Modularen Ansatzes ist die Struktur des Abhängkieten darf nicht in einem kreis enden.
