...
Die Instantiierung der Plugins über das \textit{IPLugin} Interface verfolgt einen ähnliche Ansatz und war somit seiner Zeit voraus. Jedoch hat Java aufgeholt und bietet eine leichtgewichtige und native Umsetzung des Registrierdienstes mit einer variablen Menge an möglichen Interface Implementationen.   
...
Das Modulsystem von Java bringt einen neuen Abschnitt in den Lebenszyklus einer Java Applikation, der diese wichtige Aufgabe des \textit{Plugin Managers} übernimmt. Jener erstellt einen gerichteten Graphen aus angeforderten Modulen sowie Bibliotheken und prüfte dessen Existenz auf dem Modulpfad. Wenn diese nicht existiert wird ein \textit{java.lang.module.FindException} Fehler geworfen, anderenfalls sind alle Voraussetzungen für das Starten der Applikation erfüllt und die Applikation beginnt ihre Ausführung. 
\bigbreak
...
- Renew soll mit einer minimalen Pluginanzahl auf dem Modulpfad funktionieren. Das heißt, es muss eine Ui zu sehen sein die uns zeichnungen anzulegen zun öffenn und zu speichern der netze. 
